...
impl Category {
    ...
    // Primary-only exclusion matches all subcategories with same primary.
    pub fn matches_exclusion(&self, excluded: &Category) -> bool {
        if self.primary != excluded.primary {
            return false;
        }

        match &excluded.secondary {
            Some(sec) => self.secondary.as_deref() == Some(sec.as_str()),
            None => true,
        }
    }
}
